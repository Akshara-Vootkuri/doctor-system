...
const Appointment = () => {
  ...
  // Fetch doctors if not available
  useEffect(() => {
    if (doctors.length === 0) {
      console.warn("Doctors list is empty, fetching again...");
      getDoctorsData();
      return;
    }
    const doc = doctors.find((doc) => String(doc._id) === String(docId)); // Ensure correct type match
    if (doc) {
  ...
  // Return fallback UI if doctor is not available
  if (!docInfo) {
    return <div>Doctor not available. Please try again later.</div>;
  }
